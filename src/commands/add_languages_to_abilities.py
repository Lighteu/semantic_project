...
def update_rdf_with_languages(rdf_file, tsv_file, output_file):
    ...
    with open(tsv_file, "r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            name = row["label"]
            if ability_id not in language_data:
                language_data[ability_id] = []
            language_data[ability_id].append((language, name))
    
    # Update RDF triples
    for ability in g.subjects(RDF.type, BULBA["Ability"]):
        english_name = g.value(ability, BULBA["name"])
        if english_name:
            for ability_id, translations in language_data.items():
                for lang, name in translations:
                    if lang == "English" and name == str(english_name):
                        ...
                        for lang_name, localized_name in translations:
                            ...
                            g.add((ability, BULBA["name"], Literal(localized_name, lang=lang_code)))

    # Save updated RDF to file
    g.serialize(destination=output_file, format="turtle")
    
if __name__ == '__main__':
    rdf_file = "data/python-data/rdf/abilities.ttl"
    tsv_file = "data/python-data/pokedex-i18n.tsv"
    output_file = "data/python-data/rdf/abilities.ttl"
